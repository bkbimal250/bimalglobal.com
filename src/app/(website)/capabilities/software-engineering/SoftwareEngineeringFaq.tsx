...
import type { FaqItem } from "@/content/software-engineering";

export function SoftwareEngineeringFaq({ items }: { items: readonly FaqItem[] }) {
  ...
  return (
    <div className="divide-y divide-[#171817]/15 border-y border-[#171817]/15">
      {items.map((item) => {
        ...
        const panelId = `software-engineering-faq-${item.question
          .toLowerCase()
          .replace(/[^a-z0-9]+/g, "-")}`;
