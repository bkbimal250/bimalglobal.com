...
import type { MobileFaqItem } from "@/content/mobile-platforms";

export function MobilePlatformsFaq({ items }: { items: readonly MobileFaqItem[] }) {
  ...
  return (
    <div className="divide-y divide-[#171817]/15 border-y border-[#171817]/15">
      {items.map((item) => {
        ...
        const panelId = `mobile-platforms-faq-${item.question
          .toLowerCase()
          .replace(/[^a-z0-9]+/g, "-")}`;
